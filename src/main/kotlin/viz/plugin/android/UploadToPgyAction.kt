 ...
 */
internal class UploadToPgyAction : AnAction() {
        ...
        if (psiFiles.isNotEmpty()) {
            psiFiles.forEach {
                if (it.text.contains("apply plugin: 'com.android.application'")) {
                    if (it.text.contains("apply from:'uploadApkToPgyer.gradle'")) {
                        println("uploadApkToPgyer.gradle already reference")
                        return@forEach
                    }
                    val document = PsiDocumentManager.getInstance(project).getDocument(it)
                            ?: return@forEach
                    document.setText(
                            it.text.replace(Regex("android[ ].*\\{"), "apply from:'uploadApkToPgyer.gradle'\nandroid {")
                    )
                    FileUtil.createFileFromTemplate("uploadApkToPgyer.gradle", "uploadApkToPgyer.gradle", it, project)
                    return@forEach
                }
            }
        }
