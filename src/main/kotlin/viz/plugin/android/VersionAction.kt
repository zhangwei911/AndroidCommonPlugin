 ...
 */
internal class VersionAction : AnAction() {
        ...
        if (psiFiles.isNotEmpty()) {
            psiFiles.forEach {
                if (it.text.contains("apply plugin: 'com.android.application'")) {
                    if (it.text.contains("apply from:'version.gradle'")) {
                        println("version.gradle already reference")
                        return@forEach
                    }
                    val document = PsiDocumentManager.getInstance(project).getDocument(it)
                            ?: return@forEach
                    document.setText(
                            it.text.replace(Regex("android[ ].*\\{"), "apply from:'version.gradle'\nandroid {\n" +
                                    "    customVersion()\n")
                    )
                    FileUtil.createFileBatchFromTemplate(mutableListOf(Pair("version.gradle", "version.gradle"), Pair("version.properties", "version.properties")), mutableListOf(it, it), project)
                    return@forEach
                }
            }
        }
